...
to the function: the ten with one shape - `arrayGet`, `arrayLength`, `arrayPush`, `arraySet`, `objectGet`,
`objectHas`, `objectSet`, `stringCharCodeAt`, `stringLength`, `stringSlice` - as call opcodes of their own, which the emitter chooses by a call's
